...
function configuredRedisUrl(): string | undefined {
  ...
  try {
    ...
    if (!parsed.hostname || parsed.hostname.toUpperCase() === "HOST") return undefined;
    return value;
  } catch {
    return undefined;
  }
...
function incrementLocalWindow(key: string, windowSeconds: number): number {
  const now = Date.now();
  const item = local.get(key);
  const current = !item || item.expiresAt <= now
...
export async function rateLimit(
  ...
  const key = `VEYLORIQ:rate:${scope}:${createHash("sha256").update(identity.toLowerCase()).digest("hex")}`;
  ...
  if (!url) {
    ...
    try {
      ...
      if (service.status === "wait") await service.connect();
      count = await service.incr(key);
      if (count === 1) await service.expire(key, windowSeconds);
    } catch (error) {
